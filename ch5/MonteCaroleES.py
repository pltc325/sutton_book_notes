...
'''
MonteCarole Exploring Starts Method
a) start point (s0, a0) must be chosen so that each possible pair of it can be chosen with p > 0
in case of blackjack, since s0 is given by env,hence can be viewd as randomly chosen, we only need to 
randomly choose a0 between hit and stick
b) return of each pair (s,a) is accumulated between episodes
'''
...
class BlackJackMonteCaroleAgent(object):
    ...
    def update(self, obs_with_actions, reward):
        # update q
        for ob_with_action in obs_with_actions:
            cur_sum, dealer_num, is_usable = ob_with_action[0]
            action = ob_with_action[1]
            if cur_sum < 12:
                continue
            cur_sum_index = self.cur_num_to_index[cur_sum]
            is_usable_index = self.is_usable_to_index[is_usable]
            dealer_num_index = dealer_num - 1
            self.q[action][is_usable_index][dealer_num_index][cur_sum_index] += reward
...
if __name__ == "__main__":
    ...
    for episode_num in episode_nums:
        for i in range(episode_num):
            ...
            agent.update(this_suite_obs_with_action, reward_sum)
